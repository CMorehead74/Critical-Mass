...
void CGameplayState::Update( float fElapsedTime )
{
	...
	for(int i = 0; i < 6; ++i)
	{
		if(((CDepot*)m_DepotRot[i])->GetShield() != nullptr)
		{
			...
			{
				if(!((CShield*)((CDepot*)m_DepotRot[i])->GetShield())->GetIsActive())
				{
					((CDepot*)m_DepotRot[i])->GetShield()->SetIsActive(true);
					((CDepot*)m_DepotRot[i])->GetShield()->SetIsAlive(true);
					((CShield*)((CDepot*)m_DepotRot[i])->GetShield())->Initialize(((CDepot*)m_DepotRot[i])->GetRotX(),((CDepot*)m_DepotRot[i])->GetRotY(),((CDepot*)m_DepotRot[i])->GetIsInvulnerable());
				}
			}
		}
	}
	...
	case MSG_CREATE_BULLET:
		{
			...
			if(target->GetType() == ET_BOSSENEMY)
				bullet->SetLifeTime(10.0f);
